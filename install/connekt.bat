...
if /i "%~1"=="-envparams" (
  ...
  if /i "%~1"=="-env" goto :parse_args
  if /i "%~1"=="-envname" goto :parse_args
  if /i "%~1"=="-envparams" goto :parse_args
...
for %%S in (%SCRIPT_ARGS%) do (
  for %%F in ("%%~fS") do (
    ...
    set "CMD=!CMD! --add-host=host.docker.internal:host-gateway"
    set "CMD=!CMD! -v "!DIR!:/connekt/scripts""
    if defined ENV_FILE_MOUNT set "CMD=!CMD! %ENV_FILE_MOUNT%"
    set "CMD=!CMD! %IMAGE%"
    if defined ENV_NAME_ARG set "CMD=!CMD! %ENV_NAME_ARG%"
    if defined ENV_FILE_ARG set "CMD=!CMD! %ENV_FILE_ARG%"
    set "CMD=!CMD! --script=scripts/!FILE!"
    if defined ENV_PARAM_ARGS set "CMD=!CMD! %ENV_PARAM_ARGS%"
